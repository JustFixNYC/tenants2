-- A view useful for analyzing rent history online requests.

DROP VIEW IF EXISTS dwh_rhonline;

CREATE VIEW dwh_rhonline AS (
    SELECT
        request.created_at AS request_time,
    ...
    FROM
        dwh_onlinerenthistoryrequest AS request
    LEFT OUTER JOIN
        dwh_rapidprorun AS followup_1 ON (
            followup_1.flow_uuid = %(rh_followup_1_uuid)s AND
            followup_1.user_uuid = request.user_uuid AND
            followup_1.start_time > request.created_at AND
            DATE_PART('day', followup_1.start_time - request.created_at) < %(rh_max_followup_days)s
        )
    LEFT OUTER JOIN
        dwh_rapidprorun AS followup_2 ON (
            followup_2.flow_uuid = %(rh_followup_2_uuid)s AND
            followup_2.user_uuid = request.user_uuid AND
            followup_2.start_time > request.created_at AND
            DATE_PART('day', followup_2.start_time - request.created_at) < %(rh_max_followup_days)s
        )
)
