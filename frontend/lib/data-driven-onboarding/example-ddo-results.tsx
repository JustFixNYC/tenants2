      ...
      <div className="field">
        <div className="control">
          <textarea
            ...
            onKeyDown={(e) =>
              e.ctrlKey && e.keyCode == KEY_ENTER && handleSubmit(e)
            }
...
function useDebugJsonProps<T>(router: RouteComponentProps, blankValue: T) {
  const getViewPropsStr = () =>
    (getQuerystringVar(router, QUERYSTRING_VAR) || "").trim();
  ...
  useEffect(() => setEditedValue(getViewPropsStr() || stringifiedViewProps), [
    router.location,
  ]);
    ...
    pushEditedValue() {
      router.history.push(
        router.location.pathname +
          `?${QUERYSTRING_VAR}=${encodeURIComponent(editedValue)}`
      );
    },
  };
}
...
export function ExampleDataDrivenOnboardingResults(props: RouteComponentProps) {
  ...
  return (
    <Page title="DDO results debug view" className="content">
      <div className="jf-dev-panels">
        ...
        <div className="jf-dev-panel-right">
          ...
          {dbg.err ? (
            <>
              <br />
              <pre className="has-text-danger">{dbg.err}</pre>
            </>
          ) : (
