...
import {
  ExampleMutation,
  BlankExampleInput,
  BlankSubformsExampleSubformFormSetInput,
...
function ExampleForm(props: {
  ...
  return (
    ...
    >
      {(ctx, latestOutput) => (
        <React.Fragment>
          {latestOutput && latestOutput.response && (
            <div className="notification is-success">
              Hooray, the form was submitted successfully and the response is "
              {latestOutput.response}"!
            </div>
          )}
          {ctx.nonFieldErrors &&
          ...
          <CurrencyFormField
            ...
            {...ctx.fieldPropsFor("currencyField")}
          />
          <Formset
            {...ctx.formsetPropsFor("subforms")}
            emptyForm={BlankSubformsExampleSubformFormSetInput}
          >
            {(subforms, i) => (
              <TextualFormField
                label={`example subform field #${i + 1}`}
                {...subforms.fieldPropsFor("exampleField")}
              />
            )}
          </Formset>
          <div className="field">
