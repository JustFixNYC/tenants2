...
export function customIssuesForArea(area: IssueAreaChoice, customIssues: CustomIssue[]): CustomIssue[] {
...
export function areaIssueCount(area: IssueAreaChoice, issues: IssueChoice[], customIssues: AllSessionInfo_customIssuesV2[]): number {
  return issues.reduce((total, issue) =>
    issueArea(issue) === area ? total + 1 : total,
  0) + customIssuesForArea(area, customIssues).length;
}
