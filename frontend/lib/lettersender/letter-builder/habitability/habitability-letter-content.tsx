...
export function getHabitabilityLetterContentPropsFromSession(
  ...
  const props: HabitabilityLetterContentProps = {
    ...
    issues: getIssuesFromSession(session.gceIssues as string[]),
    overchargeDetails: session.overchargeDetails ?? undefined,
  };
...
const LetterBody: React.FC<HabitabilityLetterContentProps> = (props) => {
  return (
    <>
      ...
      {props.overchargeDetails && <OverchargeSection {...props} />}
    </>
  );
};
...
const OverchargeSection: React.FC<HabitabilityLetterContentProps> = (props) => {
  return (
    <section>
      <Trans>
        <h2>Overcharge</h2>
      ...
      <p>
        <Trans id="laletterbuilder.habitability.overcharge.intro">
          I have reason to believe that the rent charged for this unit is
          overcharged.
        </Trans>
      </p>
      <p>
        <Trans id="laletterbuilder.habitability.overcharge.details">
          Details: {props.overchargeDetails ? "Yes" : "No"}
        </Trans>
      </p>
    </section>
  );
 ...
 * for easier display.
 */
function getIssuesFromSession(sessionIssues: string[]): Issue[] {
  // For GCE issues, each issue is a simple string choice
  // We'll create a simple mapping to display labels
  const gceIssueLabels: { [key: string]: string } = {
    INFORM_RIGHTS: "Good Cause Eviction Rights Information",
    OVERCHARGE: "Rent Overcharge",
    EVICTION: "Eviction Without Reason",
    NO_RENEWAL: "Lease Renewal Denied",
  };

  return sessionIssues.map((issue) => ({
    issueLabel: gceIssueLabels[issue] || issue,
    roomLabels: [], // GCE issues don't have rooms
  }));
}
