...
const LetterTitle: React.FC<LocContentProps> = (props) => (
  <letter.Title>
    <span className="is-uppercase"><Trans>Request for repairs</Trans></span>
    <letter.TitleNewline />
...
const AreaIssues: React.FC<LocContentProps> = (props) => {
  ...
  return (
    <>
      <p>
        <Trans>I need the following repairs in my home referenced below and/or in the
        common areas of the building:</Trans>
      </p>

      <h2><Trans>Repairs required</Trans></h2>
...
const AccessDates: React.FC<LocContentProps> = (props) => (
  <div className="jf-avoid-page-breaks-within">
    <h2><Trans>Available access dates</Trans></h2>
    <p>
      <Trans>Below are dates that I am available to be at my apartment to let in a
      ...
      top of this letter) in order to make arrangements with me{" "}
        <strong>at least 24 hours in advance</strong>.</Trans>
    </p>
    <ul>
...
const Requirements: React.FC<LocContentProps> = (props) => (
  <div className="jf-avoid-page-breaks-within">
    <h2><Trans>Requirements</Trans></h2>
    <Trans id="justfix.LocLetterContentSafetyRequirements"><p>
    ...
    {hasHeatIssues(props.issues) && (
      <p>
        <Trans id="justfix.LocLetterContentForHeatIssues">Please be advised that the lack of Heat and/or Hot Water constitutes an
        ...
        through the NYC Housing Court system.</Trans>
      </p>
    )}
  </div>
);
...
const PreviousReliefAttempts: React.FC<{}> = () => (
  <div className="jf-avoid-page-breaks-within">
    <h2><Trans>Previous Attempts for Relief</Trans></h2>
    <p>
      <Trans>I have already contacted 311 on several occasions, but the issue has not
      been resolved. In the meantime, I have recorded evidence of the violations
      should legal action be necessary.</Trans>
    </p>
  </div>
);
...
const LetterConclusion: React.FC<LocContentProps> = (props) => (
  <>
    <div className="jf-avoid-page-breaks-within">
      <h2><Trans>Civil penalties</Trans></h2>
      <p>
        <Trans>Pursuant to NYC Admin Code § 27-2115 an order of civil penalties for all
        existing violations for which the time to correct has expired is as
        follows:</Trans>
      </p>
      <dl>
        <Trans><dt>“C” Violation</dt>
        <dd>
        ...
        <dt>“A” Violation”</dt>
        <dd>$10-$50 one-time penalty per violation</dd></Trans>
      </dl>
    </div>
    <div className="jf-avoid-page-breaks-within">
      <p>
        <Trans>According to the NYC Admin Code § 27-2115, a civil penalty is also
        issued where a person willfully makes a false certification of
        correction of a violation per violation falsely certified.</Trans>
      </p>
      <p>
        <Trans>Please contact me as soon as possible to arrange a time to have these
        repairs made at {formatPhoneNumber(props.phoneNumber)}.</Trans>
      </p>
      <letter.Regards>
        <br />
        <br />
