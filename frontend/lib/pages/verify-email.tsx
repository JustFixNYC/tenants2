...
import {
  SessionPoller,
  SessionPollerProps,
} from "../networking/session-poller";
...
const PollEmailVerificationStatus: React.FC<Pick<
  SessionPollerProps,
  "ignoreErrors"
>> = (props) => (
  <SessionPoller
...
const StartView: React.FC<VerifyEmailProps & { successUrl: string }> = (
  props
) => {
  ...
  return (
    <Page
      title="Looks like your account has not been verified"
      withHeading
      className="content"
    >
      {/**
        ...
        {(ctx) => (
          <>
            {userHasEmail ? (
              <>
                <p>
                  Have you checked your email inbox? Check your spam folder just
                  in case.
                </p>
                <p>
                  If you can't find it, make sure your email address below is
                  correct, and re-send the verification email.
                </p>
              </>
            ) : (
              <p>
                We don't seem to have an email address associated with your
                account. Please provide a valid email address below.
              </p>
            )}
            <TextualFormField
...
const WaitingView: React.FC<VerifyEmailProps> = (props) => {
  ...
  return (
    <Page title={title} className="content has-text-centered">
      <h1>{title}</h1>
      <p>
        Don't see one in your email inbox? Check your spam folder or go back and
        try again.
      </p>
  ...
  return (
    <Page title="Thank you for verifying your account!" withHeading>
      <div className="buttons jf-two-buttons">
        ...
        <Link className="button is-primary is-medium" to={props.nextUrl}>
          Next
        </Link>
      </div>
    </Page>
    ...
    switch (view) {
      case "start":
        return (
          <StartView
            {...props}
            nextUrl={toView("waiting")}
            successUrl={toView("success")}
          />
        );
      ...
      case "waiting":
        return (
          <WaitingView
            {...props}
            prevUrl={toView("start")}
            nextUrl={toView("success")}
          />
        );
