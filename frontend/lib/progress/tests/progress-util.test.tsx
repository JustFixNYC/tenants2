...
describe("getRelativeStep()", () => {
  it("returns next step", () => {
    expect(getRelativeStep("/foo", "next", fakeSteps)).toEqual({
      path: "/foo/2",
    });
  });

  it("returns prev step", () => {
    expect(getRelativeStep("/foo/3", "prev", fakeSteps)).toEqual({
      path: "/foo/2",
    });
  });

  it("returns null on invalid step", () => {
    expect(getRelativeStep("/blah", "prev", fakeSteps)).toBeNull();
  });

  it("returns null when no previous step exists", () => {
    expect(getRelativeStep("/foo", "prev", fakeSteps)).toBeNull();
  });

  it("returns null when no next step exists", () => {
    expect(getRelativeStep("/foo/3", "next", fakeSteps)).toBeNull();
  });
});
