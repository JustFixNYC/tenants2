...
describe("removeEmptyFormsAtEnd()", () => {
  it('does nothing if not given an empty object', () => {
    expect(removeEmptyFormsAtEnd([{ foo: 'blarg' }, { foo: '' }]))
      .toEqual([{ foo: 'blarg' }, { foo: '' }]);
  });
  ...
  it('removes empty forms at end if given an empty object', () => {
    expect(removeEmptyFormsAtEnd([{ foo: 'blarg' }, { foo: '' }], { foo: '' }))
      .toEqual([{ foo: 'blarg' }]);
  });

  it('removes all forms if they are all empty', () => {
    expect(removeEmptyFormsAtEnd([{ foo: '' }, { foo: '' }], { foo: '' }))
      .toEqual([]);
  });

  it('does not remove empty forms in the middle', () => {
    expect(removeEmptyFormsAtEnd([
      { foo: 'blarg' },
      { foo: '' },
      { foo: 'narg' },
      { foo: '' },
...
describe("addEmptyForms()", () => {
  ...
  it('does nothing if not given an empty object', () => {
    expect(addEmptyForms({ items: [] })).toEqual({ initialForms: 0, items: [] });
    expect(addEmptyForms({ items: [{foo}] })).toEqual({ initialForms: 1, items: [{foo}] });
  });

  it('adds one empty form by default', () => {
    expect(addEmptyForms({ items: [{foo}], emptyForm }))
      .toEqual({ initialForms: 1, items: [{foo}, emptyForm] });
  });

  it('adds multiple empty forms if directed to', () => {
    expect(addEmptyForms({ items: [{foo}], extra: 2, emptyForm }))
  ...
  it('ignores existing empty forms at end', () => {
    expect(addEmptyForms({ items: [{foo}, emptyForm], emptyForm }))
      .toEqual({ initialForms: 1, items: [{foo}, emptyForm] });
  });
});
