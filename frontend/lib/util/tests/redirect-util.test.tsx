  ...
  it('redirects and passes along search params', () => {
    const pal = new ReactTestingLibraryPal(
      <MemoryRouter initialEntries={['/boop?flarg=1']} initialIndex={0}>
        <Switch>
          <Route path="/boop" exact component={createRedirectWithSearch('/hmm')} />
          <Route path="/hmm" exact render={(props) => <p id="hmm">hi {props.location.pathname} {props.location.search}</p>} />
        </Switch>
      </MemoryRouter>
    );
