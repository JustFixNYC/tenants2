...
import { AUTOGEN_PREAMBLE, QUERIES_PATH, BLANK_TYPE_LITERALS_PATH } from "../config";
...
export function generateBlankTypeLiterals(ctx: AutogenContext, dryRun: boolean = false): {
  filesWritten: string[]
} {
  const filesWritten: string[] = [];

  if (!fs.existsSync(BLANK_TYPE_LITERALS_PATH)) {
    fs.mkdirSync(BLANK_TYPE_LITERALS_PATH);
  }

  for (let info of ctx.typeMap.values()) {
    if (info.createBlankLiteral) {
      const type = ensureObjectType(info.type);
      const blankLiteral = JSON.stringify(createBlankTypeLiteral(type), null, 2);
      const exportedName = `Blank${type.name}`;
      const tsCode = [
        `// This file was auto-generated by querybuilder, please do not edit it.\n`,
        `export const ${exportedName} = ${blankLiteral};`
      ].join('\n');
      const fullPath = path.join(BLANK_TYPE_LITERALS_PATH, `${exportedName}.ts`);
      if (writeFileIfChangedSync(fullPath, tsCode, dryRun)) {
  ...
  reportChanged(filesWritten, (number, s) => 
    `Generated ${number} blank type literal file${s} in ${BLANK_TYPE_LITERALS_PATH}.`);
