 ...
 */
export function findStaleTypescriptFiles(
 ...
 */
export function deleteStaleTsFiles(
  graphQlFiles: GraphQlFile[] = GraphQlFile.fromDir()
): string[] {
  ...
  reportChanged(
    staleFiles,
    (number, s) => `Deleted ${number} stale TS file${s} from ${QUERIES_PATH}.`
  );
