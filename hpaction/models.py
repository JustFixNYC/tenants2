...
class HarassmentDetails(models.Model):
    ...
    user = models.OneToOneField(
        ...
        related_name="harassment_details",
...
class FeeWaiverDetails(models.Model):
    ...
    user = models.OneToOneField(
        JustfixUser,
        on_delete=models.CASCADE,
        related_name="fee_waiver_details",
    ...
    income_amount_monthly: Optional[Decimal] = models.DecimalField(
        **CURRENCY_KWARGS, help_text="The amount of income the user receives per month.", null=True
    )
    ...
    income_src_other: str = models.CharField(
        max_length=100, help_text="Other income the user receives", blank=True
    )
    ...
    def non_utility_expenses(self) -> Decimal:
        return self.expense_cable + self.expense_phone + self.expense_childcare + self.expense_other
...
class ManagementCompanyDetails(MailingAddress):
    user = models.OneToOneField(
        JustfixUser,
        on_delete=models.CASCADE,
        related_name="management_company_details",
...
class PriorCase(models.Model):
    ...
    user = models.ForeignKey(
        JustfixUser,
        on_delete=models.CASCADE,
        related_name="prior_hp_action_cases",
        help_text="The user who this prior case belongs to.",
    )

    case_number: str = models.CharField(
        max_length=9, help_text='The court case number (also known as the "index number").'
    )
    ...
    def case_type(self) -> str:
        return "&".join(
            filter(None, ["H" if self.is_harassment else "", "R" if self.is_repairs else ""])
        )
...
class HPActionDocumentsManager(models.Manager):
    ...
    def create_from_file_data(
        self, xml_data: bytes, pdf_data: bytes, id: str, **kwargs
    ) -> "HPActionDocuments":
        """
        ...
        docs = HPActionDocuments(
            xml_file=SimpleUploadedFile(f"{id}.xml", content=xml_data, content_type="text/xml"),
            pdf_file=SimpleUploadedFile(
                f"{id}.pdf", content=pdf_data, content_type="application/pdf"
            ),
            id=id,
            **kwargs,
        )
    ...
    def get_latest_for_user(
        self, user: JustfixUser, kind: Optional[str]
...
class HPActionDetails(models.Model):
    ...
    hpd_issued_violations: Optional[bool] = models.NullBooleanField(
        help_text=("Whether HPD issued any violations.")
    )
    ...
    urgent_and_dangerous: Optional[bool] = models.NullBooleanField(
        help_text=("Whether the conditions are urgent and dangerous.")
    )
...
class HPActionDocuments(models.Model):
    ...
    xml_file = models.FileField(
        upload_to="hp-action-docs/", help_text="The XML file for the HP action."
    )

    pdf_file = models.FileField(
        upload_to="hp-action-docs/", help_text="The PDF file for the HP action paperwork."
    )
    ...
    def _get_num_instruction_pages(self) -> int:
        ...
        return (
            NUM_REDHOOK_HARLEM_CJC_INSTRUCTION_PAGES
            if court
            in [
...
class UploadTokenManager(models.Manager):
    ...
    def create_for_user(self, user: JustfixUser, kind: str) -> "UploadToken":
        ...
        token = UploadToken(id=get_random_string(length=UPLOAD_TOKEN_LENGTH), kind=kind, user=user)
        token.save()
        return token
    ...
    def find_unexpired(self, token: str) -> Optional["UploadToken"]:
        ...
        return self.filter(id=token, created_at__gt=timezone.now() - UPLOAD_TOKEN_LIFETIME).first()
...
class UploadToken(models.Model):
    ...
    def create_documents_from(self, xml_data: bytes, pdf_data: bytes) -> HPActionDocuments:
        ...
        docs = HPActionDocuments.objects.create_from_file_data(
            xml_data=xml_data, pdf_data=pdf_data, kind=self.kind, user=self.user, id=self.id
        )
        self.delete()
    ...
    def get_upload_url(self) -> str:
        ...
        return absolute_reverse("hpaction:upload", kwargs={"token_str": self.id})
...
class DocusignEnvelope(models.Model):
    id = models.CharField(max_length=DOCUSIGN_ENVELOPE_ID_LENGTH, primary_key=True)
...
class ServingPapers(MailingAddress):
    ...
    name = models.CharField(
        max_length=MAX_LOB_NAME_LEN, help_text="The name of the person/company being served."
    )
    ...
    tracking_number = models.CharField(
        max_length=100,
        blank=True,
        help_text=("The tracking number for the papers."),
    )

    letter_sent_at = models.DateTimeField(
        null=True, blank=True, help_text="When the papers were mailed through the postal service."
    )
