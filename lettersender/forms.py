...
from lettersender.models import LA_MAILING_CHOICES, GCE_ISSUE_CHOICES
...
class GoodCauseIssuesForm(forms.Form):

    gce_issues = forms.MultipleChoiceField(
        required=False,
    ...
    def clean(self):
        ...
        issues = cleaned_data.get("gce_issues")
        if not issues:
            raise ValidationError(_("Please select at least one repair issue."))


class OverchargeDetailsForm(forms.Form):
    """
    Form for collecting additional details about overcharge situations.
    """
    
    overcharge_applies = forms.BooleanField(
        required=True,
        label=_("Do the following apply to your overcharge situation?"),
        help_text=_("You must answer this question to continue."),
    )

    def clean(self):
        cleaned_data = super().clean()
        overcharge_applies = cleaned_data.get("overcharge_applies")
        if overcharge_applies is None:
            raise ValidationError(_("Please answer whether the overcharge situation applies to you."))
        return cleaned_data
