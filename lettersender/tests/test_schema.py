...
from .factories import LetterSenderLetterFactory, LandlordDetailsFactory
...
class TestLetterSenderLandlordInfo:
...
class TestLetterSenderCreateAccount:
    INCOMPLETE_ERR = [
...
class TestLetterSenderSendLetter:
    SEND_QUERY = """
    ...
    def test_it_sends_letter(
        ...
        self.create_landlord_details()
        OnboardingInfoFactory(user=self.user)
        letter_obj = LetterSenderLetter(user=user, locale="es", html_content="<test/>")
        letter_obj.save()

        blank_letter = LetterSenderLetter.objects.get(
        ...
        sent_letter = LetterSenderLetter.objects.get(user=self.graphql_client.request.user)
        assert (
...
class TestLetterSenderCreateLetter:
    QUERY = """
    ...
    def test_it_creates_letter(
        ...
        assert not LetterSenderLetter.objects.filter(user=self.graphql_client.request.user).exists()

        # Create the letter
        assert self.execute()["errors"] == []

        # TODO: add tests for all the other kinds of letters too
        letter = LetterSenderLetter.objects.get(user=self.graphql_client.request.user)
...
class TestLetterSenderIssuesMutation:
    @pytest.fixture(autouse=True)
    def setup_fixture(self, graphql_client, db):
        self.user = UserFactory(email="boop@jones.net")
        graphql_client.request.user = self.user
    ...
    def test_it_raises_err_when_multiple_unsent_letter_exists(self):
        OnboardingInfoFactory(user=self.user)
        LetterSenderLetterFactory(user=self.user)
        LetterSenderLetterFactory(user=self.user)
...
class TestHabitabilityLatestLetter:
    @pytest.fixture(autouse=True)
    def setup_fixture(self, graphql_client, db):
        self.letter = LetterSenderLetterFactory(tracking_number="abcd")
        self.graphql_client = graphql_client
...
class TestLetterSenderSendOptionsMutation:
    @pytest.fixture(autouse=True)
    def setup_fixture(self, graphql_client, db):
