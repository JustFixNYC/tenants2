...
def test_landlord_details_form_restricts_name_length():
    form = LandlordDetailsFormV2(data={"name": "z" * 41})
    form.full_clean()
    assert form.errors["name"] == ["Ensure this value has at most 40 characters (it has 41)."]


def test_landlord_details_form_restricts_weird_values():
    form = LandlordDetailsFormV2(
        data={
            "name": "United States",
        }
    )
    form.full_clean()
    assert form.errors["name"] == ["This is not a valid landlord name."]


def test_form_raises_error_if_dates_are_same():
    form = AccessDatesForm(data={"date1": "2018-01-01", "date2": "2018-01-01"})
    form.full_clean()
    assert form.errors == {"__all__": ["Please ensure all the dates are different."]}


@freeze_time("2018-01-01")
def test_form_raises_error_if_dates_are_too_soon():
    form = AccessDatesForm(data={"date1": "2018-01-02", "date2": "2018-01-03"})
    form.full_clean()
...
def test_get_cleaned_dates_works():
    form = AccessDatesForm(data={"date1": "2018-01-01", "date2": "2019-02-02"})
    form.full_clean()
    assert form.errors == {}
...
@pytest.fixture
def ticket_number_formset():
    return formset_factory(TicketNumberForm, formset=TicketNumberFormset)


def test_ticket_number_formset_valid(ticket_number_formset):
    formset = ticket_number_formset(
        data={
            "form-TOTAL_FORMS": "2",
...
def test_ticket_number_formset_invalid_special_characters(ticket_number_formset):
    formset = ticket_number_formset(
        data={
            "form-TOTAL_FORMS": "2",
            "form-INITIAL_FORMS": "0",
...
def test_get_cleaned_data_with_valid_ticket_numbers(ticket_number_formset):
    formset = ticket_number_formset(
        data={
            "form-TOTAL_FORMS": "2",
            "form-INITIAL_FORMS": "0",
...
def test_get_cleaned_data_with_empty_ticket_number(ticket_number_formset):
    formset = ticket_number_formset(
        data={
            "form-TOTAL_FORMS": "2",
            "form-INITIAL_FORMS": "0",
            "form-MIN_NUM_FORMS": "0",
            "form-MAX_NUM_FORMS": "1000",
            "form-0-ticket_number": "",
            "form-1-ticket_number": "ABC123",
        }
    )
    assert formset.is_valid()
    cleaned_data = formset.get_cleaned_data(is_no_ticket_number_checked=False)
    assert cleaned_data == ["ABC123"]
