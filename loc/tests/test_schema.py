...
DEFAULT_LETTER_REQUEST_INPUT = {"mailChoice": "WE_WILL_MAIL"}
...
def test_access_dates_works(graphql_client):
    ...
    result = execute_ad_mutation(graphql_client, **{"date1": "01/02/2018"})
    assert result["errors"] == []
    assert result["session"]["accessDates"] == ["2018-01-02"]

    result = execute_ad_mutation(graphql_client, **{"date1": "2019-01-01", "date2": "2020-01-02"})
    assert result["errors"] == []
...
def test_landlord_details_address_represents_best_address(graphql_client):
    ...
    assert (
        res["data"]["session"]["landlordDetails"]["address"]
...
def test_landlord_details_are_created_when_user_has_onboarding_info(
    graphql_client, requests_mock, nycdb
):
