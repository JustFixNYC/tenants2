...
def example_doc(request, format):
    return render_document(
        request, "loc/example.html", {"now": str(datetime.datetime.now())}, format
    )
...
def envelopes(request):
    ...
    users = [
        ...
        if (
            ...
            and user.landlord_details.address_lines_for_mailing
        )
    ]
    return render_document(request, "loc/envelopes.html", {"users": users}, "pdf")
...
def render_english_to_string(
    request: Optional[HttpRequest], template_name: str, context: Dict[str, Any]
):
...
def react_render_loc_html(user, locale: str, html_comment: str = "") -> str:
    ...
    lr = react_render(SITE_CHOICES.JUSTFIX, locale, LOC_PDF_URL, ContentType.PDF, user=user)
...
def render_pdf_html(
    ...
    return render_english_to_string(
        request,
        template_name,
        {**context, "is_pdf": True, "pdf_styles_css": SafeString(pdf_styles_path.read_text())},
    )
