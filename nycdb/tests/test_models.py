...
def test_tiny_landlord_works(nycdb):
    ...
    assert boop.address.lines_for_mailing == ["124 99TH STREET", "Brooklyn, NY 11999"]
...
def test_medium_landlord_works(nycdb):
    ...
    assert mgmtco.address.lines_for_mailing == ["900 EAST 25TH STREET #2", "NEW YORK, NY 10099"]

    ll = reg.get_landlord()
    assert isinstance(ll, Company)
    assert ll.name == "LANDLORDO CALRISSIAN"
    assert ll.address.lines_for_mailing == ["9 BEAN CENTER DRIVE #40", "FUNKYPLACE, NJ 07099"]
...
class TestHPDContact:
    ...
    def test_street_address_works(self):
        ...
        assert (
            HPDContact(businesshousenumber="23", businessstreetname="blarg st").street_address
            == "23 blarg st"
        )
    ...
    def test_it_returns_none_on_db_error(self, nycdb):
        with patch.object(HPDRegistration.objects, "from_pad_bbl") as fpbblmock:
            ...
            with patch("nycdb.models.logger.exception") as loggermock:
                assert get_landlord("1234567890") is None
                loggermock.assert_called_once_with(f"Error while retrieving data from NYCDB")
