...
BOROUGH_NUMBERS = {MANHATTAN: 1, "BRONX": 2, "BROOKLYN": 3, "QUEENS": 4, "STATEN ISLAND": 5}
...
class Row(pydantic.BaseModel):
    ...
    def is_main_management_office(self) -> bool:
        return "DEVELOPMENT MANAGEMENT OFFICE" in self.FACILITY and "SATELLITE" not in self.FACILITY
...
class NychaCsvLoader:
    ...
    def load_csv(self, csvpath: Path) -> None:
        mgmt_rows = (row for row in self.iter_rows(csvpath) if row.is_main_management_office())
    ...
    def load_row(self, row: Row) -> None:
        ...
        if mgmt_org in self.offices:
            ...
            if pad_bbl not in self.pad_bbls:
                ...
                self.pad_bbls[pad_bbl] is not office and pad_bbl not in self.bbls_with_many_offices
            ):
