...
def get_old_loc_users() -> List[JustfixUser]:
    now = timezone.now()
    all_loc_users = JustfixUser.objects.filter(USER_MAILING_NEEDED_Q)\
        .prefetch_related('letter_request')
    return [
        user for user in all_loc_users
        if (now - user.letter_request.created_at) >= LOC_OLD_AGE
    ]
