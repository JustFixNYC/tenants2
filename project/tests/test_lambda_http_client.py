...
def test_hanging_without_stdout_raises_error():
    http = LambdaHttpClient('hang', '', cwd=BASE_DIR, timeout_secs=0.001, interpreter_args=[
        '-e', r'while (1) {}'])
...
def test_unparseable_port_raises_error():
    http = LambdaHttpClient('bad_port', '', cwd=BASE_DIR, interpreter_args=[
        '-e', r'console.log("SUP"); while (1) {}'])
...
def test_unicode_works(lambda_server):
    assert lambda_server.run_handler({
