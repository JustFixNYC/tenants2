...
class ZipCodeValidator(RegexValidator):
    def __init__(self):
        ...
        super().__init__(
            regex=r"^[0-9]{5}(?:-[0-9]{4})?$", message=_("Enter a valid U.S. zip code.")
        )
...
class MailingAddress(models.Model):
    ...
    urbanization = models.CharField(
        max_length=80, blank=True, help_text="Optional. Only used for addresses in Puerto Rico."
    )

    city = models.CharField(
        **CITY_KWARGS, blank=True, help_text='The city of the address, e.g. "Brooklyn".'
    )
