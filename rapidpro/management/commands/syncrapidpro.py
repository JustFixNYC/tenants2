...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        client = TembaClient(hostname, settings.RAPIDPRO_API_TOKEN)
        batches = client.get_contacts(
            after=metadata.last_sync
        ...
        for contact_batch in batches:
            ...
            for contact in contact_batch:
                ...
                if user is None:
                    continue
                self.stdout.write(f"Syncing {user} ({len(contact.groups)} groups).\n")
